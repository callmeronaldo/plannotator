...
The simplified local Git review exposes two primary modes: **`uncommitted`**, which shows tracked and untracked working-tree changes, and **`branch`**, which compares two explicitly selected refs. Branch comparison uses `git diff Branch1..Branch2`; the current checkout is not forced to be one side of the comparison. The local navigator stays on the ordinary file list rather than the Git-status/Commits view switcher. The broader VCS implementations remain available for integrations, but normal Git review exposes only these two choices.
...
The first-run Git setup dialog and the historical Sections/Commits navigation are not part of this simplified local review surface. Analysis and other provider-specific code remain separate from these two Git modes.
...
| Endpoint              | Method | Purpose                                    |
| --------------------- | ------ | ------------------------------------------ |
| `/api/diff`           | GET    | Returns `{ rawPatch, gitRef, snapshotId, origin, mode?, diffType, base, compareBranch?, hideWhitespace, gitContext, agentCwd?, semanticDiff?, callFlow?, sections?, commitInfo?, generatedFiles? }`. `snapshotId` identifies this diff snapshot; the client echoes it on `/api/diff/fresh` probes (also returned by the switch/PR endpoints). `sections` is the since-base sidecar (Committed/Changes/Untracked partition); `commitInfo` is the commit-metadata sidecar (subject, markdown body, author + avatar) present only while a `commit:<sha>` diff is active; `generatedFiles` lists the repo-relative paths that count as generated, which the client collapses by default GitHub-style (presentation-only: the patch is never filtered). Two-layer detection (`packages/shared/generated-files.ts`, vendored to Pi): built-in name defaults (`DEFAULT_GENERATED_PATTERNS` — lockfiles like `bun.lock`/`package-lock.json`/`Cargo.lock`, plus `*.min.js`/`*.min.css`/`*.map`, matched against the path's last segment) apply in every mode with no git needed, and explicit `.gitattributes` `linguist-generated` refines them in BOTH directions via one batched `git check-attr --stdin` at the review cwd (set/true marks any file, unset/false un-marks even a built-in name, unspecified keeps the default). Attribute refinement runs for plain local Git sessions only — PR worktrees, workspace, jj, GitButler, P4, and piped patches get the name-based defaults alone. Workspace mode returns `mode: "workspace"` with folder-prefixed paths and no `gitContext`. |
| `/api/diff/switch`    | POST   | Switches between `uncommitted` and two-ref `branch` comparison; body is `{ diffType, base?, compareBranch?, hideWhitespace? }`. Response includes `semanticDiff?`, `callFlow?`, `sections?`, `commitInfo?`, `generatedFiles?`, or `{ superseded: true }` when a newer concurrent switch has taken over (client ignores it). |
...
| `/api/diff/fresh`     | GET    | Cheap staleness probe: recomputes the VCS fingerprint captured with the current diff snapshot and returns `{ fresh, fingerprint?, agentCwd? }`. Accepts `?snapshot=<id>` — the client echoes the `snapshotId` it received with its diff, and a mismatch with the server's current snapshot reports stale PER CLIENT (covers cross-tab switches even when the VCS fingerprint matches). `agentCwd` re-advertises the PR checkout in PR mode. Unfingerprintable modes (e.g. P4) always report fresh to a matching snapshot. Polled by the UI's "Diff out of date · Refresh" notice. |
